$PBExportHeader$uo_multi_radio_selector.sru
forward
global type uo_multi_radio_selector from userobject
end type
type st_1 from statictext within uo_multi_radio_selector
end type
end forward
...
global uo_multi_radio_selector uo_multi_radio_selector
...
radiobutton iradb_ctrl[]		//array of radiobuttons
...
public subroutine of_getvalues (ref string as_values[])
public subroutine of_checkitem (integer ai_item)
public function integer of_init (string as_labels[], integer ai_default)
public function integer of_getchecked ()
public function string of_getcheckedtext ()
...
public subroutine of_getvalues (ref string as_values[]);integer i
string vide[]

as_values = vide

for i = 1 to upperbound(iradb_ctrl)
	as_values[i] = iradb_ctrl[i].text
next

end subroutine

public subroutine of_checkitem (integer ai_item);
if ai_item > upperbound(iradb_ctrl) then return

iradb_ctrl[ai_item].checked = true

end subroutine

public function integer of_init (string as_labels[], integer ai_default);// dynamic fill of the control with radiobuttons
//
// SeKi 2008-02-29
...
for i=1 to ii_nb_rb
	...
	if isvalid(rb) then
		...
		rb.visible = true
		rb.enabled = true
		if ib_buttons_on_right_side then rb.lefttext = true
		if i = ai_default then rb.checked = true
...
for i = 1 to upperbound(iradb_ctrl)
	if iradb_ctrl[i].checked then return i
next
...
for i = 1 to upperbound(iradb_ctrl)
	if iradb_ctrl[i].checked then return iradb_ctrl[i].text
next
...
on uo_multi_radio_selector.create
this.st_1=create st_1
this.Control[]={this.st_1}
end on

on uo_multi_radio_selector.destroy
destroy(this.st_1)
end on
